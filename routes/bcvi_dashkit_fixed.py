...
def bcvi_dashkit():
        ...
        try:
            ...
            if 'cvi_scores' not in clustering_results:
                ...
                for model in data['models']:
                    ...
                    for k in range(2, data['selected_k'] + 1):                        clustering_results['cvi_scores'][model][str(k)] = {
                            'silhouette': 0.5 + k/20,  # Increasing default values
                            'calinski_harabasz': 100 + k*10,
                            'starczewski': 0.7 + k/30,
                            'wiroonsri': 0.6 + k/25,
                            'davies_bouldin': 3.0 - k/10  # Lower is better for Davies-Bouldin
                        }
                    ...
                    for cvi_type in cvi_types:
                        try:
                            ...
                            if len(cvi_values) >= 2:  # Cần ít nhất 2 giá trị để tính BCVI
                                ...
                                logging.debug(f"Optimization type: {opt_type} for {cvi_type}")
            ...
            # Generate BCVI-based suggestions after calculation
            bcvi_based_suggestions = {}
            for model, cvi_results in bcvi_results.items():
                # Collect all BCVI values across all CVI types
                all_bcvi_values = {}
                for cvi_type, results_list in cvi_results.items():
                    for result in results_list:
                        k = result['k']
                        if k not in all_bcvi_values:
                            all_bcvi_values[k] = []
                        all_bcvi_values[k].append(result['bcvi'])
                
                # Calculate average BCVI for each k
                avg_bcvi = {}
                for k, values in all_bcvi_values.items():
                    avg_bcvi[k] = sum(values) / len(values)
                
                # Find k with highest average BCVI
                if avg_bcvi:
                    best_k = max(avg_bcvi, key=avg_bcvi.get)
                    reasoning = f"Dựa trên trung bình BCVI của tất cả chỉ số (BCVI trung bình = {avg_bcvi[best_k]:.4f})"
                    bcvi_based_suggestions[model] = {
                        'k': best_k,
                        'reasoning': reasoning,
                        'method': 'bcvi_average',
                        'avg_bcvi': avg_bcvi
                    }
                    logging.debug(f"BCVI-based suggestion for {model}: k={best_k} with avg BCVI={avg_bcvi[best_k]:.4f}")
            
            # Add BCVI-based suggestions to data
            data['bcvi_based_suggestions'] = bcvi_based_suggestions
            ...
            # Check if we have valid results after computation
            if bcvi_results and any(bcvi_results.values()):
                logging.debug("Valid BCVI results found after calculation")
                flash(f"BCVI đã được tính toán thành công cho {len(bcvi_results)} mô hình.")
            else:
                logging.error("No valid BCVI results generated")
                flash("BCVI không thể được tính toán. Vui lòng kiểm tra dữ liệu phân cụm.")
            
        except Exception as e:
            logging.error(f"Error in BCVI computation: {str(e)}")
            flash(f"Lỗi khi tính BCVI: {str(e)}")
      # Debug message to check if BCVI results exist
    ...
    if 'bcvi_results' in data:
        ...
        if not has_valid_results:
            logging.error("No valid BCVI results found despite having bcvi_results key")
            # Keep using the existing data, but log the issue
